...
% piecewise linear contour/polygon while respecting user-defined boundary 
% offset tolerance.
% 
% ******************************* IMPORTANT *******************************
...
%                     expressed in the same units as the co-ords of C. 
%                     Default setting is B_tol=Emin/2, where Emin is the
...
siz=size(C);
if numel(siz)~=2 || siz(2)>siz(1) || ~isnumeric(C) || ~ismatrix(C) || ~ismatrix(C)
    error('1st input argument (C) must be a N-by-2 array')
end

if norm(C(1,:)-C(end,:))>1E-6
